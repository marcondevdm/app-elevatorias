...
# =========================================================================
st.sidebar.image("https://img.icons8.com/fluency/96/water-pump.png", width=64)
st.sidebar.title("Configuração do Pipeline")
...
# Inicializadores de variáveis de dados
df_bruto = None
df_depara_bombas = None
df_capacidade_bombas = None
df_max_outorga = None
df_depara_niveis = None
df_nivel_medio = None
df_nivel_maximo = None
df_nivel_minimo = None

if fonte_dados == "Upload das Bases":
    st.sidebar.subheader("1. Upload dos Arquivos")
    
    file_status = st.sidebar.file_uploader(
        "📄 1. Status de Operação (CSV)",
        type=["csv"],
        help="Arquivo 'status_op_elevatorias.csv' com colunas Timestamp e TAGs."
    )
    
    file_depara = st.sidebar.file_uploader(
        "📊 2. Planilha De-Para e Capacidades (XLSX)",
        type=["xlsx"],
        help="Arquivo 'depara_elevatorias_elipse.xlsx' com as abas BOMBAS_ELIPSE, CAPACIDADE_BOMBAS, CAPACIDADE_MAX_ELEVATORIAS e RESERVATORIO_NIVEL."
    )
    
    file_nivel_med = st.sidebar.file_uploader("📈 3. Nível Médio (CSV)", type=["csv"])
    file_nivel_max = st.sidebar.file_uploader("📈 4. Nível Máximo (CSV)", type=["csv"])
    file_nivel_min = st.sidebar.file_uploader("📈 5. Nível Mínimo (CSV)", type=["csv"])

    if file_status and file_depara:
        try:
            # Leitura do CSV de operação (detectando separador)
            df_bruto = pd.read_csv(file_status, sep=None, engine='python')
            
            # Leitura das abas do Excel
            excel_file = pd.ExcelFile(file_depara)
            aba_bombas = 'BOMBAS_ELIPSE' if 'BOMBAS_ELIPSE' in excel_file.sheet_names else excel_file.sheet_names[0]
            aba_cap = 'CAPACIDADE_BOMBAS' if 'CAPACIDADE_BOMBAS' in excel_file.sheet_names else excel_file.sheet_names[0]
            aba_out = 'CAPACIDADE_MAX_ELEVATORIAS' if 'CAPACIDADE_MAX_ELEVATORIAS' in excel_file.sheet_names else excel_file.sheet_names[0]
            aba_niv = 'RESERVATORIO_NIVEL' if 'RESERVATORIO_NIVEL' in excel_file.sheet_names else excel_file.sheet_names[0]

            df_depara_bombas = pd.read_excel(excel_file, sheet_name=aba_bombas)
            df_capacidade_bombas = pd.read_excel(excel_file, sheet_name=aba_cap)
            df_max_outorga = pd.read_excel(excel_file, sheet_name=aba_out)
            df_depara_niveis = pd.read_excel(excel_file, sheet_name=aba_niv)

            # Níveis opcionais
            if file_nivel_med:
                df_nivel_medio = pd.read_csv(file_nivel_med, sep=None, engine='python')
            if file_nivel_max:
                df_nivel_maximo = pd.read_csv(file_nivel_max, sep=None, engine='python')
            if file_nivel_min:
                df_nivel_minimo = pd.read_csv(file_nivel_min, sep=None, engine='python')

            st.sidebar.success("✅ Arquivos carregados com sucesso!")
        except Exception as e:
            st.sidebar.error(f"Erro ao ler os arquivos enviados: {e}")
else:
    st.sidebar.info("💡 Carregando base sintética com elevatórias e bombas simuladas.")
...
if btn_processar:
    if df_bruto is None or df_depara_bombas is None:
        st.sidebar.error("⚠️ Por favor, carregue os arquivos obrigatórios ou utilize os dados de exemplo.")
    else:
...
if not st.session_state['dados_carregados'] or st.session_state['df_geral_analitico'].empty:
    st.info(
        ...
        "1. Selecione a origem dos dados na barra lateral à esquerda (upload dos seus arquivos ou a base de exemplo pronta).\n"
